...
} from "../../features/UnitPosseion/RegistrySlice";
...
const UpdateModal = ({ record, onClose }) => {
  const [updateRegistry, { isLoading }] = useUpdateRegistryMutation();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-emerald-700 to-emerald-600 px-6 py-4 flex items-center justify-between">
          <div>
            <h2 className="text-white font-bold text-lg">Registry Update</h2>
          ...
          <button
            onClick={onClose}
            className="text-emerald-200 hover:text-white text-2xl leading-none transition"
          >
            x
          </button>
        </div>
        ...
        {/* Info Strip */}
        <div className="bg-emerald-50 px-6 py-3 grid grid-cols-3 gap-3 border-b border-emerald-100">
          <div>
            <p className="text-xs text-emerald-400 font-semibold uppercase tracking-wide">Project</p>
            <p className="text-sm text-emerald-800 font-medium">{record?.project || "—"}</p>
          </div>
          <div>
            <p className="text-xs text-emerald-400 font-semibold uppercase tracking-wide">Unit No</p>
            <p className="text-sm text-emerald-800 font-medium">{record?.unitNo || "—"}</p>
          </div>
          <div>
            <p className="text-xs text-emerald-400 font-semibold uppercase tracking-wide">Block</p>
            <p className="text-sm text-emerald-800 font-medium">{record?.block || "—"}</p>
          </div>
        </div>
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-400"
            >
          ...
          <div>
            ...
            <label className="flex flex-col items-center justify-center w-full border-2 border-dashed border-slate-300 rounded-xl py-4 cursor-pointer hover:border-emerald-400 bg-slate-50 transition">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-400 resize-none"
            />
          </div>
        ...
        <div className="px-6 pb-6 flex gap-3">
          ...
          <button
            ...
            disabled={isLoading}
            className="flex-1 bg-emerald-700 hover:bg-emerald-800 text-white rounded-xl py-2.5 text-sm font-semibold transition disabled:opacity-50"
          >
...
// ─── MAIN PAGE ───────────────────────────────────────────────
const Registry = () => {
  const { data, isLoading, isError, error, refetch } = useGetAllRegistryQuery();
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="bg-white border border-red-200 rounded-2xl p-8 text-center shadow-sm max-w-sm">
          ...
          <button
            onClick={refetch}
            className="bg-emerald-700 text-white px-5 py-2 rounded-xl text-sm font-semibold hover:bg-emerald-800 transition"
          >
            Dobara Try Karo
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        {/* Page Header */}
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-slate-800">Registry Management</h1>
          <p className="text-slate-500 text-sm mt-1">Sabhi pending registry records yahan dikhayi denge</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between mb-6">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <input
              type="text"
              placeholder="ID, naam, project, block se search..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-300 w-full sm:w-72 shadow-sm"
            />
            <button
              onClick={refetch}
              title="Refresh"
              ...
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  {[
                    ...
                    "Block", "Unit No", "Unit Type", "Size",
                    "Agreement Value", "Date", "Status", "Action"
                  ].map((h) => (
                    <th
                      key={h}
                    ...
                    <tr key={item.id || idx} className="hover:bg-slate-50 transition">
                      ...
                      <td className="px-4 py-3">
                        <button
                          ...
                          className="bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition"
                        >
                          Update
                        </button>
                      </td>
