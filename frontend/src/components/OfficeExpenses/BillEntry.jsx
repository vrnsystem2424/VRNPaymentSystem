...
//     <th className="p-3 text-right">Planned Amt</th>  {/* ← planned/disabled */}
...
//       {/* ✅ Column 3: Planned Amount - DISABLED/READ-ONLY */}
...
//       {/* ✅ Column 4: Bill Amount - USER INPUT (same as before) */}
//       <td className="p-3">
//         <input
//           type="number"
//           value={item.amount}
...
export default function BillEntry() {
  ...
  const handleSubmit = async () => {
    // Status, Bill No., Bill Date validation
    if (!billNo.trim() || !billDate || !status) {
      alert('Status, Bill No. और Bill Date अनिवार्य हैं');
      return;
    }

    // Bill Amt validation – each item must have amount > 0
    const emptyAmountItems = items.filter(
      (item) => !item.amount || Number(item.amount) <= 0
    );
    ...
    if (emptyAmountItems.length > 0) {
      const uids = emptyAmountItems.map((i) => i.itemUid).join(', ');
      alert(`इन items का Bill Amt भरना अनिवार्य है:\n${uids}`);
      return;
    }

    const totalBasic = items.reduce((sum, i) => sum + Number(i.amount || 0), 0);
    if (totalBasic <= 0) {
      alert('कम से कम एक item का Bill Amt भरना अनिवार्य है');
      return;
    }
