...
//       .filter(({ row }) => {
//         const qColumnValue = (row[20] || '').toString().trim().toLowerCase();
...
// router.post('/Registry', upload.array('documents', 10), async (req, res) => {
//   try {
...
//     const range = `Posseion_FMS!U${matchedRowIndex}:W${matchedRowIndex}`;
...
async function findRowByID(searchId) {
  const response = await sheets.spreadsheets.values.get({
    spreadsheetId,
    range: 'Posseion_FMS!A7:W', // ✅ FIX 1 - W tak range (pehle S tha, U/V/W columns miss ho rahi thi)
  });
...
router.get('/Registry', async (req, res) => {
  try {
    const response = await sheets.spreadsheets.values.get({
      spreadsheetId,
      range: 'Posseion_FMS!A7:W', // ✅ W tak - Status(U), DocUrl(V), Remarks(W)
    });
    ...
    const data = rows
      ...
      .filter(({ row }) => {
        const uColumnValue = (row[20] || '').toString().trim().toLowerCase();
        return uColumnValue === 'pending'; // U column = index 20
      })
      .map(({ row, originalRowIndex }) => ({
        rowIndex:                originalRowIndex,
        applicationDate:         row[0]  || '',
        ...
        agreementValue:          row[13] || '',
        unitSoldThrough:         row[14] || '',
        date:                    row[15] || '', // P column
        Status:                  row[20] || '', // U column ✅ FIX 2
        Upload_Document_Copy:    row[21] || '', // V column ✅ FIX 2
        Remarks:                 row[22] || '', // W column ✅ FIX 2
      }));
  ...
  try {
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        unitSoldThrough:         row[14] || '',
        date:                    row[15] || '', // P column
        Status:                  row[20] || '', // U column ✅ FIX 3
        Upload_Document_Copy:    row[21] || '', // V column ✅ FIX 3
        Remarks:                 row[22] || '', // W column ✅ FIX 3
      },
    });
  } catch (error) {
    console.error('Error:', error.message);
...
router.post('/Registry', upload.array('documents', 10), async (req, res) => {
  try {
    ...
    // Sheet update - U=Status, V=Upload_Document_Copy, W=Remarks
    const range = `Posseion_FMS!U${matchedRowIndex}:W${matchedRowIndex}`;
