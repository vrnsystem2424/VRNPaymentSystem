...
// router.get('/Posesseion', async (req, res) => {
//   try {
//     const response = await sheets.spreadsheets.values.get({
//       spreadsheetId,
//       range: 'Posseion_FMS!A7:Y', // ✅ Y tak range lo
//     });
...
//         row,
//       }))
//       // ✅ Y column (index 24) me sirf Pending wale rows
//       .filter(({ row }) => {
//         const yColumnValue = (row[24] || '').toString().trim().toLowerCase();
//         return yColumnValue === 'pending';
//       })
...
// router.get('/Posesseion/:id', async (req, res) => {
//   try {
...
// router.post('/Posesseion', upload.array('documents', 10), async (req, res) => {
//   try {
...
//     const range = `Posseion_FMS!Y${matchedRowIndex}:AA${matchedRowIndex}`;
...
async function uploadToDrive(file) {
  ...
  const response = await drive.files.create({
    ...
    fields: 'id, webViewLink, webContentLink',
    supportsAllDrives: true,   // ✅ FIX - Shared Drive support
    supportsTeamDrives: true,  // ✅ FIX - backward compat
  });

  await drive.permissions.create({
    fileId: response.data.id,
    requestBody: { role: 'reader', type: 'anyone' },
    supportsAllDrives: true,   // ✅ FIX - permission ke liye bhi
  });
...
async function findRowByID(searchId) {
  const response = await sheets.spreadsheets.values.get({
    spreadsheetId,
    range: 'Posseion_FMS!A7:Y', // ✅ FIX - Y tak range, pehle S tha isliye ID match nahi hoti thi
  });
  ...
  try {
    ...
    const data = rows
      ...
      .map(({ row, originalRowIndex }) => ({
        ...
        agreementValue:          row[13] || '',
        unitSoldThrough:         row[14] || '',
        date:                    row[19] || '', // T column
        Upload_Document_Copy:    row[21] || '', // V column
        Remarks:                 row[22] || '', // W column
        Status:                  row[24] || '', // Y column
      }));
...
router.post('/Posesseion', upload.array('documents', 10), async (req, res) => {
  try {
    ...
    // Sheet update - Y=Status, Z=Upload_Document_Copy, AA=Remarks
    const range = `Posseion_FMS!Y${matchedRowIndex}:AA${matchedRowIndex}`;
